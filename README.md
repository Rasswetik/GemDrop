...
The Upgrade page has its own feed of the last 15 successful spins (`GET /api/upgrade/recent-wins`): stake image and TON value, target gift PNG and TON value, and the exact chance from the settled spin. Wager wins are labeled as progress toward the original gift instead of an awarded target. A losing spin shows cashback and, if generated, the personal promo code and its reward in the result window, with a button leading to Bonuses.
