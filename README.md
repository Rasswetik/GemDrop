...
On a fresh database, twenty starter levels are seeded once. After that the level list is dynamic: Admin → **Уровни и награды** can add or delete levels, edit thresholds/rewards, and deleted levels are not recreated after restart. Level 1 starts at zero turnover; starter level N uses `N × (N − 1) / 2` TON in paid stakes. Rewards can be TON balance, Portal gift, wager gift, generated personal code, or generated deposit bonus code. Turnover increases when a Mines or Upgrade stake is settled and when a Roll spin is paid, including gift-backed stakes at their stored TON value. Deposits and administrative balance adjustments do not increase turnover. Players claim unlocked rewards exactly once from the profile progress panel. Reward tiles are displayed as three fixed-width squares per row in the scrolling dialog. Codes generated as level rewards have one activation and can be redeemed by anyone who knows the code; the player can reopen a claimed level to view its code.
...
The Roll source and admin settings remain in the project, but the Roll screen is inaccessible from the app navigation. **Апгрейд** replaces its bottom tab. The player chooses a TON stake (0.10–1,000 TON, limited by balance) or an inventory gift worth up to 1,000 TON, then a more expensive Portal target of any catalog price. There is no minimum chance; the maximum remains 80%. Chance = `Upgrade RTP × stake price / target price`; the default RTP is 90% and can be adjusted in Admin → **RTP игры**. Very small chances are settled using an exact integer ratio, never rounded to zero or rounded up to a higher probability. A TON stake is debited atomically with the spin; an ordinary gift stake consumes that gift, and a win awards the target gift. The server uses secure randomness and request IDs prevent double settlement.
...
The Upgrade page has its own successful-spin feed (`GET /api/upgrade/recent-wins`) loaded from the current feed cutoff without a “show more” button: stake image and TON value, target gift PNG and TON value, and the exact chance from the settled spin. Wager wins are labeled as progress toward the original gift instead of an awarded target. An ordinary losing stake shows cashback and, if generated, the personal promo code and its reward in the result window, with a button leading to Bonuses. A lost wager-locked gift never receives cashback or a compensation promo.

The Mines and Upgrade feeds include the largest actual win of the current Moscow day. Admin → **Последние победы** clears either feed or both by recording cutoffs; game records, inventory, transactions, and player balances are retained. An active Mines round that wins after clearing is shown. Upgrade loads the whole current feed immediately without a “show more” button and refreshes as soon as each spin settles. Cashback displays only the credited TON amount. Personal compensation promos are more likely on substantial losses; for losses of at least 100 TON the third eligible loss without a promo guarantees one, while losses of 10–99.99 TON guarantee one on the fifth and losses of 2–9.99 TON on the eighth. Wager gift losses remain ineligible.

Admin → **Пользователи → [user]** has a dedicated personal promo form. An optional custom code can be entered, or the server generates a unique one. Balance, deposit bonus, gift and wager gift rewards are supported; codes are assigned to the chosen player and appear in their Bonuses. The deposit dialog keeps amount and confirmation up front, with optional promo and wallet address inside expandable sections.
...
В апгрейде после проигрыша со ставкой TON сохраняются сумма ставки и цель. Когда оставшегося баланса недостаточно, прокрутка недоступна до пополнения. При ставке подарком и после выигрыша выбор сбрасывается. Уведомления показываются поверх модальных окон. Оформление встроено в `templates/index.html`: открытие главной страницы не требует файла `static/css/gemdrop-ui.css` на сервере и не зависит от кэша CSS. При изменении отдельного CSS-файла перенесите его содержимое во встроенный блок `gemdrop-ui` перед развёртыванием.

Инвентарь профиля показывает по три карточки в ряд. PNG вписывается в область без обрезки, название переносится целиком, цена остаётся под ним.

## Update 17 — premium emoji / bot settings / visual polish

Подробное описание правок находится в `UPDATE_17.md`. Сборка сохраняет основу версии 15, добавляет постоянный каталог Telegram custom emoji, админский сценарий определения ID через бота, импорт Telegram-поста в черновик, custom emoji на inline-кнопках, редактируемое приветствие `/start` и обновлённый единый визуальный слой.
