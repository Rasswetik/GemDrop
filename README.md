...
The Roll source and admin settings remain in the project, but the Roll screen is inaccessible from the app navigation. **Апгрейд** replaces its bottom tab. A player chooses an inventory gift and a more expensive Portal catalog target. Only targets whose true calculated chance is between 1% and 80% appear and can be spun. The chance is `Upgrade RTP × source price / target price` (90% RTP by default) and is drawn as the green share of the wheel. Admin → **RTP игры** adjusts Upgrade RTP. Wager gifts can also be upgraded: on a win the new target remains wager locked with the same multiplier, preserved progress and a new target equal to its new price times that multiplier; on a loss the original gift is consumed. The server picks the outcome with cryptographically secure randomness. Each request ID can settle only once; the source gift is consumed, and a successful target gift is added to inventory. This follows the existing virtual inventory and withdrawal workflow; it does not purchase an NFT on Portal automatically.
