...
В карточке пользователя администратор может выдать личную копию готового промокода либо создать новый промокод любого доступного типа с автоматически заполненным ID игрока. Каждый выданный код отдельный и одноразовый. В профиле кнопка «Бонусы» показывает число доступных личных промокодов; счётчик обновляется при открытии приложения, возвращении в него и периодически при открытой странице. Инвентарь отсортирован по стоимости, а фон карточки зависит от цены подарка.
