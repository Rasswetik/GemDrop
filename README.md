...
Задайте `DATA_DIR=/opt/render/project/src/data` и подключите постоянный диск к этому пути. Диск `/opt/render/project/src/data` сохраняет `gemdrop.sqlite3`, `portal_gifts.json`, сопоставление ID подарков и последние пять резервных копий каталога после перезапусков. При неудачной синхронизации старый каталог остаётся доступен. На бесплатном плане Render постоянный диск недоступен; не запускайте этот проект там, если нужно сохранять балансы.
