...
The Upgrade page has its own feed of the last 15 successful spins (`GET /api/upgrade/recent-wins`): stake image and TON value, target gift PNG and TON value, and the exact chance from the settled spin. Wager wins are labeled as progress toward the original gift instead of an awarded target. An ordinary losing stake shows cashback and, if generated, the personal promo code and its reward in the result window, with a button leading to Bonuses. A lost wager-locked gift never receives cashback or a compensation promo.

The Mines and Upgrade feeds include the largest actual win of the current Moscow day. Admin → **Последние победы** clears either feed or both by recording cutoffs; game records, inventory, transactions, and player balances are retained. An active Mines round that wins after clearing is shown. Upgrade initially displays three recent rows with a button for more, and refreshes as soon as each spin settles. Cashback displays only the credited TON amount. Personal compensation promos are more likely on substantial losses; for losses of at least 100 TON the third eligible loss without a promo guarantees one, while losses of 10–99.99 TON guarantee one on the fifth and losses of 2–9.99 TON on the eighth. Wager gift losses remain ineligible.
